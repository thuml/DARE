...
if __name__ == '__main__':
    ...
    for i in range(0, len(train_npy), 102400):
        ...
        np.save('../data/tmall/train_' + name + '_' + str(len(train_npy[i:i + 102400])) + '.npy', saved_train_array)
        train_data_index += 1

    saved_val_array = np.asarray(val_npy, dtype=object)
    np.save('../data/tmall/val.npy', saved_val_array)
    saved_test_array = np.asarray(test_npy, dtype=object)
    np.save('../data/tmall/test.npy', saved_test_array)
